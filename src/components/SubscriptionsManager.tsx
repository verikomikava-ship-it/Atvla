...
export const SubscriptionsManager: React.FC<SubscriptionsManagerProps> = ({
  ...
  return (
    <div className="space-y-2">
      ...
      {/* სტატისტიკა */}
      <Card className="bg-teal-500/10 border-teal-700/50">
        <CardContent className="p-2 space-y-1">
          <div className="flex justify-between text-xs">
            <span className="flex items-center gap-1 text-teal-300"><Check className="h-2.5 w-2.5" />გადახდილი:</span>
            <span className="font-bold text-teal-300">{subsPaid}₾</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-teal-400">დარჩენილი:</span>
            <span className="font-bold text-teal-400">{subsRemaining}₾</span>
          </div>
          <div className="flex justify-between text-xs text-teal-200 pt-1.5 border-t border-teal-700">
            <span>სულ:</span>
            <span className="font-bold">{subsTotal}₾</span>
          </div>
          {subsTotal > 0 && <Progress value={(subsPaid / subsTotal) * 100} indicatorClassName="bg-teal-400" className="h-1 bg-teal-900/50" />}
        </CardContent>
      </Card>
      ...
      <div className="space-y-2">
        {monthlySubs.length === 0 ? (
          <p className="text-center text-slate-500 py-4">ამ თვეში გამოწერა არ დამატებულა</p>
        ) : (
          monthlySubs.map((sub) => {
            ...
            if (sub.id === editingId) {
              return (
                <Card key={sub.id} style={{ borderLeftColor: bgColor, borderLeftWidth: '4px' }} className="bg-teal-500/10 border-teal-700/50">
                ...
                <CardContent className="p-2 flex justify-between items-center">
                  <label className="flex items-center cursor-pointer flex-1">
                    ...
                    <div className={cn(sub.paid && 'line-through opacity-50')}>
                      <p className="font-bold text-xs text-teal-300">{sub.name}</p>
                      <p className="text-[10px] text-teal-200">{sub.amount}₾</p>
                  ...
                  <div className="flex items-center gap-0.5 ml-1">
                    <Button variant="ghost" size="icon" onClick={() => startEdit(sub)} className="h-6 w-6 text-slate-400 hover:text-slate-200"><Pencil className="h-3 w-3" /></Button>
                    <Button variant="ghost" size="icon" onClick={() => handleRemove(sub.id, sub.name, sub.amount)} className="h-6 w-6 text-red-400 hover:text-red-300 hover:bg-red-500/10"><X className="h-3 w-3" /></Button>
                  </div>
                </CardContent>
              </Card>
            );
